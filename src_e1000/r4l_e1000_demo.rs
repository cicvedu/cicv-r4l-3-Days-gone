...
impl net::DeviceOperations for NetDevice {
    ...
    fn stop(_dev: &net::Device, _data: &NetDevicePrvData) -> Result {
        ...
        let irq_handler = _data._irq_handler.load(core::sync::atomic::Ordering::Relaxed);
        if !irq_handler.is_null() {
            let irq_handler = unsafe { Box::from_raw(irq_handler) };
        }

        _data.napi.as_ref().disable();
...
struct E1000DrvPrvData {
    ...
    _bar: i32,
    _pci_dev_ptr: Arc<*mut bindings::pci_dev>,
}
...
impl pci::Driver for E1000Drv {
    ...
    fn probe(dev: &mut pci::Device, id: core::option::Option<&Self::IdInfo>) -> Result<Self::Data> {
        ...
        Ok(Box::try_new(E1000DrvPrvData {
            ...
            _bar: bars,
            _pci_dev_ptr: Arc::try_new(dev.as_pci_ptr())?,
        })?)
    }
    ...
    fn remove(data: &Self::Data) {
        ...
        let dev = netdev_reg.dev_get();
        let pci_dev_ptr = data._pci_dev_ptr.as_ref(); // & *mut bingings::pci_dev

        // free the bar memory
        unsafe {
            let bar = data._bar;
            bindings::pci_release_selected_regions(*pci_dev_ptr, bar);
